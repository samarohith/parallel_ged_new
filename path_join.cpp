...
void opt_order(void)
{
	...
	for(int i = 0; i < gdb_size; i += thread_work)
	{
		tp.push(opt_order_join, i);
	}
}
void opt_order_join( int id, int start )
{
	unsigned* tested = new unsigned[gdb_size], test_cnt = -1, free_end = 0;
	int* backup = new int[gdb_size], ans = 0, dist_cand = 0, cand = 0;
	set<unsigned>::const_iterator ip;
...
#endif
	/* indexed graphs */
	for (unsigned i = start, j = gdb_size - 1; i != j && i < start + thread_work && i < gdb_size;) {
		//cout<<i<<" "<<gdb_size<<endl;
		switch (status[i])
		{
		case 0: 
			for (vector<iter_graph>::const_iterator iv = git[i].begin(), end = git[i].end(); iv != end; ++ iv) {
	...
	if(num == gdb_size-2)
	{
		cout<<"inside unindexed graphs"<<endl;
		free_end = 0;
		for (unsigned i = 0, m = uid.size(); i != m; ++ i) {
			for (unsigned j = i + 1, n = uid.size(); j != n; ++ j) {
				if (uid[j] > free_end) {
